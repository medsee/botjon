...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "⚡ Leverage: `3x` | 🟢 Long + 🔴 Short\n"
        "🛡 Sliv yo'q | ⏱ 24/7 ishlaydi\n"
        "📡 EMA + RSI + BB + MACD + ADX\n\n"
...
async def start_trading_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if trading_bot and trading_bot.running:
        await msg.reply_text(
            ...
            "Holat tugmasini bosib tekshiring.",
            parse_mode="Markdown",
            reply_markup=get_keyboard(),
        )
        return
    ...
    if not api_key or api_key == "your_mexc_api_key_here":
        await msg.reply_text(
            ...
            "`.env` faylga `MEXC_API_KEY` qo'shing.",
            parse_mode="Markdown",
        )
        return
...
async def status_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for sym, pos in trading_bot.positions.items():
        ...
        pos_text += f"\n  {side_icon} `{sym}` @ `${pos.entry_price:,.4f}` ({pos.age_seconds:.0f}s)"
...
async def positions_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for sym, pos in trading_bot.positions.items():
        ...
        pnl_now   = ""
        text += (
            f"{side_icon} *{sym}* `{pos.leverage}x`\n"
...
async def settings_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    sl  = float(os.getenv("STOP_LOSS_PCT",       "0.008"))  * 100
    tp  = float(os.getenv("TAKE_PROFIT_PCT",     "0.020"))  * 100
    tr  = float(os.getenv("MAX_TRADE_PCT",       "0.06"))   * 100
    mp  = os.getenv("MAX_OPEN_POSITIONS", "5")
    ml  = float(os.getenv("MAX_DAILY_LOSS_PCT",  "0.06"))   * 100
    lev = os.getenv("LEVERAGE", "3")
